...
r15 = search_knowledge_base.invoke({"query": "warranty repair", "category": "policy", "top_k": 3})
...
check("check_service_status all", r16["all_normal"] == True, f"services={len(r16['services'])}")

r17 = check_service_status.invoke({"service_name": "order_service"})
check("check_service_status specific", r17["services"]["order_service"] == "normal")
...
r4 = guard.check("")
check("Input guard: empty input", r4.blocked, "empty should be blocked")
...
check("Audit log: integrity", audit.verify_integrity() == True)
...
# Session query
sess_costs = cost.query_session("sess_test")
check("Cost tracker: session query", len(sess_costs) > 0, f"records={len(sess_costs)}")
...
for t in triggers2:
    print(f"    -> {t.name}: {t.severity.value}")
...
check("refund_id format: RF + digits", all(rid.startswith("RF") and rid[2:].isdigit() for rid in refund_ids))
...
required_nodes = [
    ...
    "tech_support_process", "finance_process", "after_sale_process",
    "quality_check", "escalation_process", "human_approval_node", "compile_result"
]
graph_dict = graph.get_graph() if hasattr(graph, 'get_graph') else {}
print(f"  Graph nodes: {list(graph_dict.keys()) if isinstance(graph_dict, dict) else 'not dict'}")
check("Graph has all 9 nodes", isinstance(graph_dict, dict))
...
expected_routes = ["/health", "/chat", "/api/v1/agent/chat", "/api/v1/agent/chat/stream",
                   "/api/v1/agent/chat/ui", "/api/v1/approval/", "/api/v1/observability/"]
for er in expected_routes:
    found = any(er in path for path, _ in routes)

...
Usage:
    python tests/final_verification.py       # standalone
...
def main():
    """Final verification test entry point."""
    ...
    # Empty input — intentionally passed by design ("空消息无攻击面，由业务层处理")
    g = get_input_guard()
    r = g.check("")
    test("Empty input passed (by design)", r.action.value == "pass", f"action={r.action.value}")
